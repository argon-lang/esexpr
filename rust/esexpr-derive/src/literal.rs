use syn::{token, LitBool, LitFloat, LitInt, LitStr, Ident};
...
fn generate_literal(literal: ESExprLiteral) -> TokenStream {
    match literal {
        ESExprLiteral::Constructor(constructor) =>
            generate_constructor(constructor),
        ESExprLiteral::Scalar(scalar) =>
            generate_scalar(scalar),
        ESExprLiteral::Null(level) =>
            generate_null(level),
    }
}
...
fn generate_scalar(scalar: ESExprScalarLiteral) -> TokenStream {
    match scalar {
        ESExprScalarLiteral::Bool(bool) =>
            quote! {
                ::esexpr::ESExpr::Bool(#bool)
            },
        ESExprScalarLiteral::Str(str) =>
            quote! {
                ::esexpr::ESExpr::Str(::std::borrow::Cow::Borrowed(#str))
            },
        ESExprScalarLiteral::Int(int) =>
            quote! {
