...
fn read_expr_plus<'a>(
	...
	Ok(ExprPlus::Expr(match token {
		...
		ExprToken::ConstructorStartKnown(name) => read_expr_constructor(iter, string_pool, name)?,
		...
		ExprToken::AppendStringTable => {
			...
			let new_string_table =
				AppendedStringPool::decode_esexpr(new_string_table).map_err(ParseError::InvalidStringPool)?;

			match new_string_table {
				AppendedStringPool::Fixed(mut fixed_string_pool) => string_pool.append(&mut fixed_string_pool.strings),
